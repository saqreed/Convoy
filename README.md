...
- Конвои
  ...
  - preview открытого конвоя до вступления
  - nearby discovery с фильтрами по статусу, времени старта и длине маршрута
...
- Форум
  - долгоживущие обсуждения внутри конвоя
  - закрепление важных постов лидером
  - редактирование/удаление автором или лидером
- Голосования (polls)
...
Самый воспроизводимый способ поднять весь стек локально:
...
Backend-контейнер автоматически выполняет `prisma migrate deploy` перед запуском сервера.
...
GitHub Actions workflow в `.github/workflows/ci.yml` проверяет:

- backend install, Prisma generate, migrations, typecheck и build
- frontend install, lint и production build
- валидность `docker-compose.yml` и сборку Docker images
